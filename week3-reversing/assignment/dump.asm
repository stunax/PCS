...
08048719 <phase_1>:
; standard
 8048719:	55                   	push   ebp ; saves old ebp
 804871a:	89 e5                	mov    ebp,esp ; sets new ebp 
 804871c:	83 ec 18             	sub    esp,0x18 ; reserves 0x18 bytes
 loal var = 0x80493e7
 804871f:	c7 45 f4 e7 93 04 08 	mov    DWORD PTR [ebp-0xc],0x80493e7
 ;start of loop. Loop checks one character at a time, if user password is the same as stored password
 8048726:	8b 45 08             	mov    eax,DWORD PTR [ebp+0x8] 
 ; adds one to argument 1. so it points to next character in password
 8048729:	8d 50 01             	lea    edx,[eax+0x1] ; adress of eax + 1
 ...
 ; checks if first password corresponds to the password at eax. jumps past explode call if's the same
...
08048763 <phase_2>:
 ...
 ; makes sure there is more than 5 matches
 ...
 ;last element of password moved to eax
 80487c0:	8b 45 f4             	mov    eax,DWORD PTR [ebp-0xc]
 80487c3:	8b 54 85 dc          	mov    edx,DWORD PTR [ebp+eax*4-0x24] ; current argument
 ...
 80487d1:	0f af 45 f4          	imul   eax,DWORD PTR [ebp-0xc] ; mult previous argument by i
 80487d5:	39 c2                	cmp    edx,eax ; check that they are the same
 ...
 ;check if i is less or equal to 5
...
080487fa <phase_3>:
 ...
 ;allocated memory
 8048800:	8d 45 ec             	lea    eax,[ebp-0x14] ; pointer to var 1
 8048803:	50                   	push   eax ; variable from string
 8048804:	8d 45 f0             	lea    eax,[ebp-0x10] ; pointer to var 2
 8048807:	50                   	push   eax ; pointer to format string ; %d %d
 8048808:	68 44 94 04 08       	push   0x8049444
 ;scan input and put result in local memory
 804880d:	ff 75 08             	push   DWORD PTR [ebp+0x8]
 8048810:	e8 db fc ff ff       	call   80484f0 <sscanf@plt>
 8048815:	83 c4 10             	add    esp,0x10
 ; check that 2 or more matches
 8048818:	83 f8 01             	cmp    eax,0x1
 804881b:	7f 05                	jg     8048822 <phase_3+0x28>
 804881d:	e8 35 fe ff ff       	call   8048657 <explode>
 8048822:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 ; Checks that first values is greater than 999 insigned
 8048825:	3d e7 03 00 00       	cmp    eax,0x3e7
 804882a:	77 05                	ja     8048831 <phase_3+0x37>
 804882c:	e8 26 fe ff ff       	call   8048657 <explode>
 ; set var 3 to 0
 8048831:	c7 45 f4 00 00 00 00 	mov    DWORD PTR [ebp-0xc],0x0
 8048838:	eb 27                	jmp    8048861 <phase_3+0x67>
 ;start of loop ; load char from user input into eax
 804883a:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 ;checks if it's 1
 804883d:	83 e0 01             	and    eax,0x1
 ;if (eax bitand 1) == 0
 8048840:	85 c0                	test   eax,eax
 8048842:	74 11                	je     8048855 <phase_3+0x5b>
 ; else
 ; val 2 + val 2 + val 2 + 1 to local val 2
 8048844:	8b 55 f0             	mov    edx,DWORD PTR [ebp-0x10]
 8048847:	89 d0                	mov    eax,edx
 8048849:	01 c0                	add    eax,eax
 804884b:	01 d0                	add    eax,edx
 804884d:	83 c0 01             	add    eax,0x1
 8048850:	89 45 f0             	mov    DWORD PTR [ebp-0x10],eax
 8048853:	eb 08                	jmp    804885d <phase_3+0x63>
 ; if 
 ; load first char from user input, shifts it 1 right, and replace with new value
 8048855:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 8048858:	d1 e8                	shr    eax,1
 804885a:	89 45 f0             	mov    DWORD PTR [ebp-0x10],eax
 ; out of ifelse
 ; local var 3++
 804885d:	83 45 f4 01          	add    DWORD PTR [ebp-0xc],0x1
 ; load char from user input
 8048861:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 8048864:	83 f8 01             	cmp    eax,0x1
 ; check that it is not 1
 8048867:	75 d1                	jne    804883a <phase_3+0x40>
 ; check that the second argument to scan is the amount of 0 bits before the first 1 bit. The value can get scrambled making it alot harder.
 8048869:	8b 45 ec             	mov    eax,DWORD PTR [ebp-0x14]
 804886c:	3b 45 f4             	cmp    eax,DWORD PTR [ebp-0xc]
 804886f:	74 05                	je     8048876 <phase_3+0x7c>
 8048871:	e8 e1 fd ff ff       	call   8048657 <explode>
...
08048888 <func4>:
 ; tribonachi
 ; fn(x) = if x > 2 {fn(x-1) + fn(x-2) + fn(-3)} else 1
 8048888:	55                   	push   ebp
 8048889:	89 e5                	mov    ebp,esp
 ;save ebx
 804888b:	53                   	push   ebx
 804888c:	83 ec 04             	sub    esp,0x4
 804888f:	83 7d 08 02          	cmp    DWORD PTR [ebp+0x8],0x2 ; check if argument 1 < 2. jump if so
 8048893:	7f 07                	jg     804889c <func4+0x14>
 ; else return arg 1
 8048895:	b8 01 00 00 00       	mov    eax,0x1 ; eax = 1
 804889a:	eb 3c                	jmp    80488d8 <func4+0x50>
 ; arg 1 - 1 as argument for recursive call
 804889c:	8b 45 08             	mov    eax,DWORD PTR [ebp+0x8]
 804889f:	83 e8 01             	sub    eax,0x1
 80488a2:	83 ec 0c             	sub    esp,0xc
 80488a5:	50                   	push   eax
 80488a6:	e8 dd ff ff ff       	call   8048888 <func4>
 80488ab:	83 c4 10             	add    esp,0x10
 80488ae:	89 c3                	mov    ebx,eax
 80488b0:	8b 45 08             	mov    eax,DWORD PTR [ebp+0x8]
 ; arg 1 - 2 as argument for recursive call
 80488b3:	83 e8 02             	sub    eax,0x2
 80488b6:	83 ec 0c             	sub    esp,0xc
 80488b9:	50                   	push   eax
 80488ba:	e8 c9 ff ff ff       	call   8048888 <func4>
 80488bf:	83 c4 10             	add    esp,0x10
 80488c2:	01 c3                	add    ebx,eax
 ;save result of recursive call to ebx
 80488c4:	8b 45 08             	mov    eax,DWORD PTR [ebp+0x8]
 ; arg 1 - 3 as argument for recursive call
 80488c7:	83 e8 03             	sub    eax,0x3
 80488ca:	83 ec 0c             	sub    esp,0xc
 80488cd:	50                   	push   eax
 80488ce:	e8 b5 ff ff ff       	call   8048888 <func4>
 80488d3:	83 c4 10             	add    esp,0x10
 ; add all recursive results together
 80488d6:	01 d8                	add    eax,ebx
 ; return ebx to state before call
 80488d8:	8b 5d fc             	mov    ebx,DWORD PTR [ebp-0x4]
 80488db:	c9                   	leave  
 80488dc:	c3                   	ret    

080488dd <phase_4>:
 ; test if tribonach(user input 1) == 289329
 ; standard stuff
 80488dd:	55                   	push   ebp
 80488de:	89 e5                	mov    ebp,esp
 80488e0:	83 ec 18             	sub    esp,0x18
 80488e3:	83 ec 04             	sub    esp,0x4
 ;push pointer to var 1
 ...
 ; test that 1 or more matches on sscanf
 80488fa:	85 c0                	test   eax,eax
 80488fc:	7f 05                	jg     8048903 <phase_4+0x26>
 80488fe:	e8 54 fd ff ff       	call   8048657 <explode>
 ; take user input 1 and calls it as argument for func 4
 8048903:	8b 45 f4             	mov    eax,DWORD PTR [ebp-0xc]
 8048906:	83 ec 0c             	sub    esp,0xc
 8048909:	50                   	push   eax
 804890a:	e8 79 ff ff ff       	call   8048888 <func4>
 804890f:	83 c4 10             	add    esp,0x10
 ;check that result = 289329, which is tribonachi(22)
 8048912:	3d 31 6a 04 00       	cmp    eax,0x46a31
 8048917:	74 05                	je     804891e <phase_4+0x41>
 8048919:	e8 39 fd ff ff       	call   8048657 <explode>
...
08048930 <phase_5>:
 ; offset input based on what character it is
 ; ebp-0x27 = var 1
 8048930:	55                   	push   ebp
 8048931:	89 e5                	mov    ebp,esp
 8048933:	53                   	push   ebx
 8048934:	83 ec 24             	sub    esp,0x24 ; allocate array?
 8048937:	8d 45 d9             	lea    eax,[ebp-0x27] ; pointer to array 1
 804893a:	b9 1b 00 00 00       	mov    ecx,0x1b ; 27 bytes in array?
 804893f:	bb 00 00 00 00       	mov    ebx,0x0 ; 0
 8048944:	89 18                	mov    DWORD PTR [eax],ebx ; var 1 = 0
 8048946:	89 5c 08 fc          	mov    DWORD PTR [eax+ecx*1-0x4],ebx ; last element = 0 
 804894a:	8d 50 04             	lea    edx,[eax+0x4]
 ;load pointer to first real argument of array
 ; removes the first two bits from edx ; minus up to 4
 804894d:	83 e2 fc             	and    edx,0xfffffffc
 ; get last 2 bits of eax
 8048950:	29 d0                	sub    eax,edx
 ; 27 + bit offset?
 8048952:	01 c1                	add    ecx,eax
 ; and to remove bit offset ?
 8048954:	83 e1 fc             	and    ecx,0xfffffffc
 8048957:	83 e1 fc             	and    ecx,0xfffffffc
 ; fills array with 0
 804895a:	b8 00 00 00 00       	mov    eax,0x0
 804895f:	89 1c 02             	mov    DWORD PTR [edx+eax*1],ebx
 8048962:	83 c0 04             	add    eax,0x4
 8048965:	39 c8                	cmp    eax,ecx
 8048967:	72 f6                	jb     804895f <phase_5+0x2f>
 ; add offset to eax?
 8048969:	01 c2                	add    edx,eax
 ; set local var 1 = 0 ; it's a counter ?
 804896b:	c7 45 f4 00 00 00 00 	mov    DWORD PTR [ebp-0xc],0x0
 8048972:	eb 26                	jmp    804899a <phase_5+0x6a>
 ; fill local array with values from arg 1 array
 ; loop til var 1 = 19
 8048974:	8b 55 f4             	mov    edx,DWORD PTR [ebp-0xc]
 ; load var 1
 8048977:	8b 45 08             	mov    eax,DWORD PTR [ebp+0x8]
 ; load arg 1 and add var 1 + arg 1 ; arg1[counter]
 804897a:	01 d0                	add    eax,edx
 ; move current byte from arg1 to eax
 804897c:	0f b6 00             	movzx  eax,BYTE PTR [eax]
 804897f:	0f be c0             	movsx  eax,al
 ; keep only 4 smallest bits
 8048982:	83 e0 0f             	and    eax,0xf
 ; Use these bits as offset for modified input string
 8048985:	0f b6 80 c0 ad 04 08 	movzx  eax,BYTE PTR [eax+0x804adc0]
 804898c:	8d 4d d9             	lea    ecx,[ebp-0x27]
 804898f:	8b 55 f4             	mov    edx,DWORD PTR [ebp-0xc]
 8048992:	01 ca                	add    edx,ecx
 ; pointer to current byte in local array
 8048994:	88 02                	mov    BYTE PTR [edx],al
 ; set that byte to byte from input
 ; increase counter and check if it is 19
 8048996:	83 45 f4 01          	add    DWORD PTR [ebp-0xc],0x1
 804899a:	83 7d f4 19          	cmp    DWORD PTR [ebp-0xc],0x19
 804899e:	7e d4                	jle    8048974 <phase_5+0x44>
 ...
 80489ab:	50                   	push   eax
 ; compare modified input with stored string
 ...
 80489d0:	c9                   	leave  
 80489d1:	c3                   	ret    
 ; part that gets called later
 ; arranges input from user so it
  80489d2:	55                   	push   ebp
...
08048a22 <func6>:
 ...
 8048a2e:	8b 45 fc             	mov    eax,DWORD PTR [ebp-0x4]
 8048a31:	8b 00                	mov    eax,DWORD PTR [eax]
 8048a33:	8d 48 01             	lea    ecx,[eax+0x1]
 8048a36:	8b 55 fc             	mov    edx,DWORD PTR [ebp-0x4]
 8048a39:	89 0a                	mov    DWORD PTR [edx],ecx
...
08048a4a <phase_6>:
 ...
 ;push six pointers as argument for sscanf
 ...
 8048a69:	68 4c bb 04 08       	push   0x804bb4c ; format string
 8048a6e:	68 18 94 04 08       	push   0x8049418 ; string
 8048a73:	ff 75 08             	push   DWORD PTR [ebp+0x8]
 8048a76:	e8 75 fa ff ff       	call   80484f0 <sscanf@plt>
 8048a7b:	83 c4 20             	add    esp,0x20
 ; check that there is more than 5 matches from sscanf
 8048a7e:	83 f8 05             	cmp    eax,0x5
 8048a81:	7f 05                	jg     8048a88 <phase_6+0x3e>
 8048a83:	e8 cf fb ff ff       	call   8048657 <explode>
 8048a88:	83 ec 04             	sub    esp,0x4
 ; pointer to val 6 ; start of array?
 8048a8b:	8d 45 d8             	lea    eax,[ebp-0x28] ; 
 8048a8e:	50                   	push   eax
 8048a8f:	68 22 8a 04 08       	push   0x8048a22
 8048a94:	68 e8 ad 04 08       	push   0x804ade8
 ; call part of phase 5
 8048a99:	e8 34 ff ff ff       	call   80489d2 <phase_5+0xa2>
 8048a9e:	83 c4 10             	add    esp,0x10
 8048aa1:	c7 45 f4 00 00 00 00 	mov    DWORD PTR [ebp-0xc],0x0
 8048aa8:	eb 15                	jmp    8048abf <phase_6+0x75>
 8048aaa:	8b 45 f4             	mov    eax,DWORD PTR [ebp-0xc]
 8048aad:	8b 44 85 dc          	mov    eax,DWORD PTR [ebp+eax*4-0x24]
 8048ab1:	3b 45 f4             	cmp    eax,DWORD PTR [ebp-0xc]
 8048ab4:	74 05                	je     8048abb <phase_6+0x71>
 8048ab6:	e8 9c fb ff ff       	call   8048657 <explode>
 8048abb:	83 45 f4 01          	add    DWORD PTR [ebp-0xc],0x1
 8048abf:	83 7d f4 05          	cmp    DWORD PTR [ebp-0xc],0x5
 8048ac3:	7e e5                	jle    8048aaa <phase_6+0x60>
