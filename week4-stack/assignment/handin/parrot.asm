...
080484cc <parse>:
 ...
 ; load stack smashing cookie
 80484d8:	65 a1 14 00 00 00    	mov    eax,gs:0x14
 ;save it on stack after buffer
 80484de:	89 45 f4             	mov    DWORD PTR [ebp-0xc],eax
 ; remove cookie
 80484e1:	31 c0                	xor    eax,eax
 ; load local var 1 to eax
 80484e3:	8b 45 b4             	mov    eax,DWORD PTR [ebp-0x4c]
 ;save local var 1 to local var 2
 ; save eax to local var 2
 80484e6:	89 45 c4             	mov    DWORD PTR [ebp-0x3c],eax
 ; set local var 3 to 0
 80484e9:	c7 45 f0 00 00 00 00 	mov    DWORD PTR [ebp-0x10],0x0
 ;goto loop check
 80484f0:	eb 1b                	jmp    804850d <parse+0x41>
 ;start of loop
 ; eax = loc3
 80484f2:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 ; ecx = loc2
 80484f5:	8b 4d c4             	mov    ecx,DWORD PTR [ebp-0x3c]
 ; edx = loc3
 80484f8:	8b 55 f0             	mov    edx,DWORD PTR [ebp-0x10]
 ; edx = loc2 + loc3
 80484fb:	01 ca                	add    edx,ecx
 ; keep last part of bytes
 80484fd:	0f b6 12             	movzx  edx,BYTE PTR [edx]
 ; save current character in string at string[loc3]
 8048500:	88 54 05 c8          	mov    BYTE PTR [ebp+eax*1-0x38],dl
 ;increase counter and save on stack
 8048504:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 8048507:	83 c0 01             	add    eax,0x1
 804850a:	89 45 f0             	mov    DWORD PTR [ebp-0x10],eax
 ;check loop
 ; load local var 2 into edx
 804850d:	8b 55 c4             	mov    edx,DWORD PTR [ebp-0x3c]
 ; load local var 3 into eax
 8048510:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 ; eax = loc2 + loc3
 8048513:	01 d0                	add    eax,edx
 ; only keep first byte in eax
 8048515:	0f b6 00             	movzx  eax,BYTE PTR [eax]
 ; check that current character is null byte, and jump to start of loop if it's not
 8048518:	84 c0                	test   al,al
 804851a:	75 d6                	jne    80484f2 <parse+0x26>
 ;end of loop
 ;load loc3
 804851c:	8b 45 f0             	mov    eax,DWORD PTR [ebp-0x10]
 ;load pointer to loc2
 804851f:	8d 55 c4             	lea    edx,[ebp-0x3c]
 ; increase poiinter by 4 to point further in
...
08048551 <main>:
 ...
 ;if arg1 == 2;
 ; prints you gave me "input". not bad
 8048585:	8b 45 0c             	mov    eax,DWORD PTR [ebp+0xc]
 ; add 4 to pointer at arg 2
 8048588:	83 c0 04             	add    eax,0x4
 ;get new value that it points to
 804858b:	8b 00                	mov    eax,DWORD PTR [eax]
 ; Add that value to esp and call parse
 804858d:	89 04 24             	mov    DWORD PTR [esp],eax
 8048590:	e8 37 ff ff ff       	call   80484cc <parse>
 ;test that parse returns 0, if so, let eax be 0, else set it to 1
